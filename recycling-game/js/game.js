class LoginScene extends Phaser.Scene {
    ...
    create() {
        ...
        loginButton.on('pointerdown', () => {
            ...
            if (username) {
                localStorage.setItem('username', username);  // 'username'을 사용하고 있는지 확인
...
class ScoreScene extends Phaser.Scene {
    ...
    create() {
        ...
        for (let i = 0; i < 10; i++) {
            if (scores[i]) {
                this.add.text(750, 200 + i * 60, `${i + 1}. ${scores[i].name}: ${scores[i].score} point`, { fontSize: '32px', fill: '#000000' }).setOrigin(0.5);
            }
        }
